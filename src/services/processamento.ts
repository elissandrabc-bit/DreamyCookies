...
export function calcularMetricasGlobais(ranking: RankingCookie[]): MetricasGlobais {
  if (ranking.length === 0) {
    return {
      faturamentoTotal: 0,
      unidadesVendidas: 0,
  ...
  const acumulado = ranking.reduce(
    (acc, item) => {
...
export function filtrarCategoriasPorNome(
  ...
  return categorias.filter((cat) =>
    cat.categoria_nome.toLowerCase().includes(busca)
  );
}

/** Map: categorias prontas para a tabela */
export function mapearCategorias(categorias: VendaCategoria[]): CategoriaFormatada[] {
...
export function mapearRankingCookies(ranking: RankingCookie[]): CookieRankingFormatado[] {
...
export function mapearPedidos(pedidos: PedidoResumo[]): PedidoFormatado[] {
...
export function obterProdutoDestaque(
  ranking: RankingCookie[]
): { nome: string; quantidade: number } | null {
  if (ranking.length === 0) {
    return null;
  }

  const contagem = ranking.reduce<Record<number, { nome: string; qtd: number }>>(
    (acc, item) => {
  ...
  const destaque = valores.reduce((maior, atual) =>
    atual.qtd > maior.qtd ? atual : maior
  );
