...
import {
  ...
  filtrarCategoriasPorNome,
  mapearCategorias,
  mapearPedidos,
  mapearRankingCookies,
  obterProdutoDestaque,
} from './services/processamento.js';
import {
  limparDashboard,
  renderizarErro,
...
async function carregarDashboard(): Promise<void> {
  ...
  try {
    ...
    const termoFiltro =
      filtroInput instanceof HTMLInputElement ? filtroInput.value : '';

    const categoriasFiltradas = filtrarCategoriasPorNome(
      dadosDashboard.vendas_por_categoria,
      termoFiltro
    );
